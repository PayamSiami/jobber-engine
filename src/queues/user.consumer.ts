...
import { IBuyerDocument, ISellerDocument, winstonLogger } from '@jobber/shared';
...
import { publishDirectMessage } from '@jobber/queues/user.producer';
...
const consumeReviewFautMessages = async (channel: Channel): Promise<void> => {
  try {
    if (!channel) {
      channel = (await createConnection()) as Channel;
    }
    ...
    log.log('error', 'UsersService UserConsumer consumeReviewFautMessages() method error:', error);
  }
};
...
export { consumeBuyerDirectMessage, consumeSellerDirectMessage, consumeReviewFautMessages, consumeSeedGigDirectMessages };
