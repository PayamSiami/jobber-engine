...
import { IHitsTotal, IPaginateProps, IQueryList, ISearchResult, ISellerGig } from '@jobber/shared';

export async function gigById(index: string, gigId: string): Promise<ISellerGig> {
  const gig: ISellerGig = await elasticSearch.getDocumentById(index, gigId);
  return gig;
}
