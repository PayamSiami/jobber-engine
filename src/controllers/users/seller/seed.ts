...
import { BadRequestError, IBuyerDocument, IEducation, IExperience, ISellerDocument } from '@jobber/shared';
...
export class SellerSeed {
  public async seller(req: Request, res: Response): Promise<void> {
    ...
    for (let i = 0; i < buyers.length; i++) {
      ...
      const checkIfSellerExist: ISellerDocument | null = await sellerService.getSellerByEmail(`${buyer.email}`);
